|feature_name|type|description|sample_value|changes|released_from|released_to|
|------------|----|-----------|------------|-------|-------------|-----------|
|behavior_application_changes_count_bn_2d|JSON|A number of changed fields between all applications that the user applied during the last two days. Typos in text fields are ignored (up to levenstein distance of 2 - roughly 2 mistakes).|<code lang="json">{"borrower_typeOfLiving":2}</code>|v2.00.0 - Rename of behaviorApplicationChanges.|v2.00.0||
|behavior_application_changes_count_bn_1w|INT|A number of changed fields between all applications that the user applied during the last week. Typos in text fields are ignored (up to levenstein distance of 2 - roughly 2 mistakes).|3||v2.06.0||
|behavior_application_changes_count_bn_1m|INT|A number of changed fields between all applications that the user applied during the last month. Typos in text fields are ignored (up to levenstein distance of 2 - roughly 2 mistakes).|3||v2.06.0||
|behavior_application_changes_count_bn_1y|INT|A number of changed fields between all applications that the user applied during the last year. Typos in text fields are ignored (up to levenstein distance of 2 - roughly 2 mistakes).|3||v2.06.0||
|behavior_application_changes_count_fields_bn_2d|JSON|The list of changed webdata fields and number of changes for last two days by birth number|||v2.02.0||
|behavior_application_changes_count_fields_sa_2d|JSON|The list of changed webdata fields and number of changes for last two days from this device.|||v2.02.0||
|behavior_application_changes_count_sa_2d|INT|A number of changed fields amongst all applications that were applied from this device during the last two days. Typos in text fields are ignored (up to levenstein distance of 2 - roughly 2 mistakes).|3||v2.00.0||
|behavior_application_changes_count_sa_1w|INT|A number of changed fields amongst all applications that were applied from this device during the last week. Typos in text fields are ignored (up to levenstein distance of 2 - roughly 2 mistakes).|3||v2.06.0||
|behavior_application_changes_count_sa_1m|INT|A number of changed fields amongst all applications that were applied from this device during the last month. Typos in text fields are ignored (up to levenstein distance of 2 - roughly 2 mistakes).|3||v2.06.0||
|behavior_application_changes_count_sa_1y|INT|A number of changed fields amongst all applications that were applied from this device during the last year. Typos in text fields are ignored (up to levenstein distance of 2 - roughly 2 mistakes).|3||v2.06.0||
|behavior_application_changes_detail_bn_2d|JSON|Changed values that user made between all applications that applied during the last two days. Typos in text fields are ignored (up to levenstein distance of 2 - roughly 2 mistakes).|```{"income":[24000,30000,28000]}```||v2.00.0||
|behavior_application_changes_detail_sa_2d|JSON|Changed values between all applications that applied during the last two days from this device. Typos in text fields are ignored (up to levenstein distance of 2 - roughly 2 mistakes).|```{"income":[24000,30000,28000]}```||v2.02.0||
|behavior_applications_count_all_max|INT|A number of all applications that are connected with current application by any kind of identificator (any of long-term cookie, birthnumber, name+surname, email, phone number).|5||v2.00.0||
|behavior_duration_form|LONG|Amount of time [ms] user spends in the form. (It's a duration between event finishedForm and first hit on first form page.|179123|v2.00.0 - Rename of behaviorFormTime.|v2.00.0||
|behavior_duration_web_said|LONG|Amount of time [ms] user spends in the website. It's a time of the last hit on the website subtracted from the first hit on the first page.|47|v2.00.0 - Rename of behaviorPageViewTime.|v2.00.0||
...
|behavior_emails_count_bn_1m|INT|A number of different email addresses that given user used for applications in last month.|2|v2.04.1 - Rename of behavior_email_count_bn_1m|v2.04.1||
...
|behavior_pages_visited_count_said|INT|A number of visited pages during the session.|5|v2.00.0 - Rename of behaviorPagesVisited.|v2.00.0||
|behavior_pages_visited_said|JSON|Visited pages during the user's session.|'/pages/list.php,/pages/list2.php'||v2.00.0||
|behavior_slider_move_count|INT|A number of a slider moves during the form filling.|3|v2.00.0 - Rename of of behaviorSliderMoves.|v2.00.0||
|behavior_slider_move_time|LONG|The time [ms] that the user spends interacting with the slider during the form filling.|13245|v2.00.0 - Rename of of behaviorSliderMoveTime.|v2.00.0||
...
|behavior_source_referrer|STRING|The URL that referred traffic to the website.|https://www.google.com/|v2.00.0 - Rename of referrer.|v2.00.0||
...
|behavior_timer_detail|JSON|The time [ms] measured by custom timers.|```{"terms1":235448, "about_us": 13987}```|v2.00.0 - Rename of behaviorTermsVisitedTime.|v2.00.0||
...
|behavior_typing_correcting_mistakes_count|INT|A number of correction of mistakes during form filling. It's a sum of all pressed delete and backspace keys.|1|v2.00.0 - Rename of behaviorCorrectingMistakes.|v2.00.0||
...
|behavior_typing_dwell_time_mean|FLOAT|The average duration [ms] between pressing and releasing a single key.|87.8|v2.00.0 - Rename of behaviorTypingDwellTimeAvg.|v2.00.0||
...
|behavior_typing_fields_typed_into_count|INT|A number of filled fields during the form filling.|14|v2.00.0 - Rename of of behaviorFilledFields.|v2.00.0||
...
|behavior_typing_flight_time_mean|FLOAT|The average duration [ms] between pressing key1 and pressing key2.|87.8|v2.00.0 - Rename of behaviorFlightTimeAvg.|v2.00.0||
...
|behavior_typing_latency_time_mean|FLOAT|The average duration [ms] between pressing down of key1 and releasing key2.|87.8|v2.00.0 - Rename of behaviorTypingLatencyTimeAvg.|v2.00.0||
...
|behavior_typing_paste_count|INT|The user pastes some content in an element during the form filling.|3|v2.00.0 - Rename of behaviorCopyPaste.|v2.00.0||
|behavior_typing_paste_count_fields|JSON|A number of a paste event for every form field.|```{"email":1}```||v2.00.0||
|behavior_typing_speed|INT|A number of keystrokes per minute.|227|v2.00.0 - Rename of behaviorTypingSpeed.|v2.00.0||
...
|behavior_typing_time_fields|JSON|A duration [ms] that user spends in the input field. Data is for every field in a form.|```{"address":944}```|v2.00.0 - Rename of behavior_typing_time_in_field_fields|v2.00.0||
...
|device_browser|STRING|The name and version of the user's browser.|Firefox|v2.00.0 - Rename of deviceBrowserType and deviceBrowserVersion.|v2.00.0||
...
|device_category|STRING|The type of device: desktop, tablet, mobile or TV.|Desktop|v2.00.0 - Rename of of deviceCategory.|v2.00.0||
...
|device_mobile_brand|STRING|The name of a mobile manufacturer.|Apple|v2.00.0 - Rename of of deviceBrand.|v2.00.0||
|device_mobile_price|INT|Manufacturers suggested a retail price [EUR] at the release date of a device.|250|v2.00.0 - Rename of deviceMobilPrice.|v2.00.0||
|device_mobile_release_date|STRING|Year the device was released.|2018|v2.00.0 - Rename of deviceMobileReleaseYear.|v2.00.0||
...
|device_mobile_type|STRING|Mobile device model name.|Samsung S9|v2.00.0 - Rename of deviceMobilType.|v2.00.0||
|device_net_name|STRING|The official network name to which the user is connected to.|CERBEROSNET||v2.06.0||
|device_operating_system|STRING|Name of the user's operating system and its version.|Windows 10|v2.00.0 - Combination of deviceOperatingSystem and deviceOperatingSystemVersion.|v2.00.0||
...
|device_screen_resolution|STRING|Resolution of user's screen.|1376x768|v2.00.0 - Rename of deviceScreenResolution.|v2.00.0||
...
|device_tor|BOOL|Indicating whether a user is using Tor. Tor is a software that allows users to browse the web anonymously.|True|v2.00.0 - Rename of deviceTorNetworkUsage.|v2.00.0||
...
|location_distance_address_and_geoip|INT|The approximate distance [km] between the provided contact address (or permanent if no contact address is given) and location derived from IP address.|45|v2.00.0 - Rename of of locationDifference.|v2.00.0||
...
|location_geoip_city|STRING|User's city, derived from his IP address.|Praha|v2.00.0 - Rename of deviceCity.|v2.00.0||
|location_geoip_lat|FLOAT|The approximate latitude of the user's location, derived from his IP address.|14.5104|v2.00.0 - Rename of deviceLatitude.|v2.00.0||
|location_geoip_lon|FLOAT|The approximate longitude of the user's location, derived from his IP address.|50.0765|v2.00.0 - Rename of deviceLongitude.|v2.00.0||
...
|person_insolvency_isir|BOOL|Indicates that the person was found in the insolvency register.|False|v2.00.0 - Rename of of insolvency.|v2.00.0||
